...
class BillingPage extends BasePage {
    ...
    async fillBillingCard(cardFullName, billingCardNum, billingExpiryMonth, billingExpiryYear, billingCvv) {
    ...
    async fillBillingAddress(billingStreet1, billingStreet2, billingCity, billingState, billingZip) {
    ...
    async completePayment(ghostFlow) {
        if (ghostFlow) {
            await this.clickPayButton();
        }
        else {
            await this.clickAgreePolicy();
            await this.clickPayButton();
        }
    }
}
